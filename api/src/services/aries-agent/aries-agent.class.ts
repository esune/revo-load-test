...
export class AriesAgent {
  ...
  private async init() {
    ...
    logger.info("Connecting to holder...");
    const connection = await this.newConnection();
    const url =
      "http://holder:8034/connections/receive-invitation?auto_accept=true";
    await Axios.post(url, connection.invitation);
    logger.info("Invitation sent to holder.");
  }
  ...
  private async revokeCredential(
    ...
    const url = `${this.acaPyUtils.getAdminUrl()}/issue-credential/revoke?cred_rev_id=${revocation_id}&rev_reg_id=${revoc_reg_id}&publish=${publish}`;
    const response = await Axios.post(
      url,
      null,
      this.acaPyUtils.getRequestConfig()
